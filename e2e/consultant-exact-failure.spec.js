...
test('exact live failure: route only then hotel+car+search same turn', async ({ page }) => {
    ...
    transcript.push({ role: 'user', text: 'i want to go melbourne from sydney on the 28th of august' }, { role: 'aleya', text: reply });
    ...
    transcript.push({ role: 'user', text: 'ill need hotel and car hire . yes begin your search' }, { role: 'aleya', text: reply });
    ...
    fs.writeFileSync(path.join(ARTIFACTS, 'exact-live-failure.json'), JSON.stringify({
        transcript,
        stateAfterFirst: {
