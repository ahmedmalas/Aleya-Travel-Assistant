...
test('browser proof: SYD→MEL projection and product_default 1 adult', async ({ page }) => {
    ...
    await page.evaluate(() => {
        ...
        localStorage.setItem('aleya-travel:conversation:v5', JSON.stringify({ schemaVersion: 5, state }));
    });
    await page.reload();
    ...
    await expect(page.getByTestId('search-travellers')).toHaveAttribute('data-traveller-source', 'product_default');
