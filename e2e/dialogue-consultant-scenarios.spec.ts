...
async function dumpProof(
  ...
  const payload = {
    ...
    searchSession: search
      ...
      : null,
    inventedPricesAvailabilityOrBookings: traces.every(
      (t) => t.inventedPricesAvailabilityOrBookings === false,
    ),
    canonicalModifiedByValidatedActionsOnly: traces.every(
      (t) => t.canonicalModifiedByValidatedActionsOnly === true,
    ),
  };
  fs.writeFileSync(
...
test.describe('Dialogue consultant — seven scenarios', () => {
  ...
  test('Scenario 2 — Hotel refinement', async ({ page }) => {
    ...
    expect(reply).not.toMatch(/Shall I start the live search/i);
    transcript.push(
      { role: 'user', text: 'Find me hotels around Docklands.' },
      { role: 'aleya', text: reply },
    );
    ...
    expect(reply).not.toMatch(FORBIDDEN);
    transcript.push(
      { role: 'user', text: 'Something luxurious but still good value.' },
      { role: 'aleya', text: reply },
    );
  ...
  test('Scenario 3 — Result reference', async ({ page }) => {
    ...
    await sendChat(
      page,
      'I like the second hotel. Are there better flights that arrive earlier?',
    );
  ...
  test('Scenario 7 — Misspellings', async ({ page }) => {
    ...
    await sendChat(
      page,
      'i need accomodation neer docklands somthing nice but not too exspensive',
    );
    const reply = await lastAleya(page);
