 ...
 *
 * Phase 19L: accepts explicit zero answers (`0 children`, `no children`,
 * `There are no children`, `We have no children`) in domain 0–99. Repair
...
function parseChildCountRepairToken(raw: string): number | null {
  const value = parseChildCountToken(raw);
...
function isBlockedChildCountMessage(message: string): boolean {
  ...
  if (
    ...
    /\bno\s+infants?\b/i.test(message)
  ) {
    return true;
  }
  if (
  ...
  // Whole-message "no children" is accepted earlier; any other "no children"
  // context (repair fragments, policy text) stays blocked.
  if (/\bno\s+child(?:ren)?\b/i.test(message)) {
    return true;
  }
...
/** Phase 19L — whole-message explicit zero child answers only. */
const ZERO_CHILD_COUNT_MESSAGE =
  /^(?:(?:we\s+have|there\s+are)\s+)?no\s+child(?:ren)?[.!]*$/i;
...
function extractExplicitChildCount(message: string): number | null {
  ...
  const repaired = extractPassengerCountRepairToken(
    ...
    'child(?:ren)?',
    'child(?:ren)?',
    parseChildCountRepairToken,
  );
  if (repaired !== null) {
    return repaired;
  }

  if (ZERO_CHILD_COUNT_MESSAGE.test(text)) {
    return 0;
  }
