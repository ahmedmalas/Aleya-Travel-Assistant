...
describe('phase 6E — HikingWalkingRequestedConversationStateExtractor skeleton', () => {
  ...
  it('is included once in the production composite after snow activities and before empty', () => {
    const extractors = readExtractors(
      createConversationStateExtractor() as CompositeConversationStateExtractor,
    );

    expect(extractors).toHaveLength(22);
    ...
    expect(emptyIndexes).toEqual([21]);
    ...
    expect(extractors[21]).toBeInstanceOf(EmptyConversationStateExtractor);
  });
