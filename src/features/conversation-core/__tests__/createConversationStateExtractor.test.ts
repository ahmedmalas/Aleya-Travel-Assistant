...
describe('phase 5E/5J — createConversationStateExtractor factory', () => {
  ...
  it('separate factory calls return separate composites with destination, origin, departure-date, return-date, adult-count, child-count, infant-count, flights-requested, accommodation-requested, car-hire-requested, activities-requested, restaurants-requested, nearby-discovery-requested, beaches-requested, camping-requested, kayaking-requested, four-wheel-driving-requested, scenic-drives-requested, attractions-requested, snow-activities-requested, then empty extractors', () => {
    ...
    expect(firstExtractors).toHaveLength(21);
    expect(secondExtractors).toHaveLength(21);
    ...
    expect(firstExtractors[20]).toBeInstanceOf(EmptyConversationStateExtractor);
    ...
    expect(secondExtractors[20]).toBeInstanceOf(EmptyConversationStateExtractor);
