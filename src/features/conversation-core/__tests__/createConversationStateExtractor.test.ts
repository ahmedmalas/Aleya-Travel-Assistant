...
describe('phase 5E/5J — createConversationStateExtractor factory', () => {
  ...
  it('separate factory calls return separate composites with destination, origin, departure-date, return-date, adult-count, child-count, infant-count, flights-requested, then empty extractors', () => {
    ...
    expect(firstExtractors).toHaveLength(9);
    expect(secondExtractors).toHaveLength(9);
    ...
    expect(firstExtractors[8]).toBeInstanceOf(EmptyConversationStateExtractor);
    ...
    expect(secondExtractors[8]).toBeInstanceOf(EmptyConversationStateExtractor);
