...
const CONVERSATION_ID = 'conversation-core-shopping-requested-001';
...
describe('phase 3W — explicit shoppingRequested only', () => {
  it('initial shoppingRequested is null', () => {
    const state = createInitialConversationCoreState({
      conversationId: CONVERSATION_ID,
      now: CREATED_AT,
    });
    expect(state.shoppingRequested).toBeNull();
  });
  ...
  it('explicit true is stored', () => {
    ...
    const result = turn('I want shopping', initial, 0, {
  ...
  it('explicit false is stored', () => {
    ...
    const withTrue = turn('need shops', initial, 0, {
      shoppingRequested: true,
    });
    expect(withTrue.state.shoppingRequested).toBe(true);

    const withFalse = turn('no markets', withTrue.state, 1, {
      shoppingRequested: false,
    });
    expect(withFalse.state.shoppingRequested).toBe(false);
    expect(withFalse.state.shoppingRequested).not.toBeNull();
  });
  ...
  it('omission preserves a previous true', () => {
    ...
    const second = turn(
      'shopping centres outlets places to shop',
      first.state,
      1,
    );
  ...
  it('user message text can re-enable but cannot clear via removal wording (Phase 19B)', () => {
    ...
    const afterWords = turn('shopping shops markets', withTrue.state, 1);
    expect(afterWords.state.shoppingRequested).toBe(true);
  ...
  it('all previous request flags and canonical fields are preserved', () => {
    ...
    const second = turn('no shopping', first.state, 1, {
      shoppingRequested: false,
    });
    expect(second.state.shoppingRequested).toBe(false);
  ...
  it('existing transcript behaviour remains unchanged', () => {
    ...
    const second = turn('shopping shops markets', first.state, 1);
    expect(second.state.shoppingRequested).toBe(true);
