...
describe('conversation-core architectural boundary', () => {
  it('visible chat → public index → processConversationTurn → canonical empty state', () => {
    ...
    expect(types).not.toMatch(/eventsRequested\?: boolean \| null/);
    ...
    expect(index.includes('ExplicitGuestCountConversationStateExtractor')).toBe(
      false,
    );
    ...
    expect(
      processTurn.includes('ExplicitGuestCountConversationStateExtractor'),
    ).toBe(false);
    ...
    expect(types).not.toMatch(/eventsRequested: boolean \| null/);
    expect(types).not.toMatch(/eventsRequested: null,/);
    expect(applyUpdate).not.toMatch(/stateUpdate\?\.eventsRequested/);
    expect(types).toMatch(/nightlifeRequested: boolean \| null/);
    ...
    // Phase 18F / 19I / 19J — contextual extractors may read currentState.
    ...
    expect(bareNumberPassengerCountExtractor).toMatch(
      /export class BareNumberPassengerCountConversationStateExtractor/,
    );
    expect(bareNumberPassengerCountExtractor).toContain('Phase 19I');
    expect(bareNumberPassengerCountExtractor).toMatch(
      /input: ConversationStateExtractionInput/,
    );
    expect(bareNumberPassengerCountExtractor).toMatch(/input\.message/);
    expect(
      bareNumberPassengerCountExtractor.includes('input.currentState'),
    ).toBe(true);
    const explicitGuestCountExtractor = readSrc(
      'src/features/conversation-core/ExplicitGuestCountConversationStateExtractor.ts',
    );
    expect(explicitGuestCountExtractor).toMatch(
      /export class ExplicitGuestCountConversationStateExtractor/,
    );
    expect(explicitGuestCountExtractor).toContain('Phase 19J');
    expect(explicitGuestCountExtractor).toMatch(
      /input: ConversationStateExtractionInput/,
    );
    expect(explicitGuestCountExtractor).toMatch(/input\.message/);
    expect(explicitGuestCountExtractor.includes('input.currentState')).toBe(
      true,
    );
    expect(explicitGuestCountExtractor).toContain(
      'isAccommodationGuestCountFollowUpActive',
    );
    expect(restaurantPreferenceExtractor).toMatch(
  ...
  it('keeps the Phase 10K reply catalogue internal and wording-only', () => {
    ...
    expect(catalogue).toContain('Phase 10U');
    expect(catalogue).toContain('Phase 10V');
    expect(catalogue).toContain('Phase 10W');
    expect(catalogue).toContain('Phase 10X');
    expect(catalogue).toContain('Phase 10Y');
    expect(catalogue).toContain('Phase 11C');
    ...
    expect(catalogue).toContain('Perfect — ${adultCount} adult travelling.');
    expect(catalogue).toContain('Perfect — ${adultCount} adults travelling.');
    expect(catalogue).toContain('Perfect — ${childCount} child travelling.');
    expect(catalogue).toContain('Perfect — ${childCount} children travelling.');
    expect(catalogue).toContain('Perfect — ${infantCount} infant travelling.');
    expect(catalogue).toContain('Perfect — ${infantCount} infants travelling.');
    ...
    expect(catalogue.includes('newlyDisabledRequestFlags')).toBe(false);
