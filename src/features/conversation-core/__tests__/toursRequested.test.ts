...
const CONVERSATION_ID = 'conversation-core-tours-requested-001';
...
describe('phase 3T — explicit toursRequested only', () => {
  it('initial toursRequested is null', () => {
    const state = createInitialConversationCoreState({
      conversationId: CONVERSATION_ID,
      now: CREATED_AT,
    });
    expect(state.toursRequested).toBeNull();
  });
  ...
  it('explicit true is stored', () => {
    ...
    const result = turn('I want a tour', initial, 0, {
      toursRequested: true,
    });
  ...
  it('explicit false is stored', () => {
    ...
    expect(withTrue.state.toursRequested).toBe(true);

    const withFalse = turn('no guided tour', withTrue.state, 1, {
      toursRequested: false,
    });
    expect(withFalse.state.toursRequested).toBe(false);
    expect(withFalse.state.toursRequested).not.toBeNull();
  });
  ...
  it('omission preserves a previous true', () => {
    ...
    const second = turn('day tour sightseeing tour', first.state, 1);
  ...
  it('user message text cannot clear or change an existing value', () => {
    ...
    const afterWords = turn('tour tours guided tour', withTrue.state, 1);
    expect(afterWords.state.toursRequested).toBe(true);
  ...
  it('all previous request flags and canonical fields are preserved', () => {
    ...
    const second = turn('no tours', first.state, 1, {
      toursRequested: false,
    });
    expect(second.state.toursRequested).toBe(false);
  ...
  it('existing transcript behaviour remains unchanged', () => {
    ...
    const second = turn('tour tours guided tour', first.state, 1);
    expect(second.state.toursRequested).toBe(true);
