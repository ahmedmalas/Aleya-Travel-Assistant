...
export function UserProfilePanel({ section }: { section: ProfileSection }) {
  ...
  return (
    <section className="px-6 pb-8" aria-labelledby={`profile-${section}-title`}>
      <button type="button" onClick={() => window.dispatchEvent(new CustomEvent('aleya-account-back'))} className="mb-5 text-sm text-sky-300 hover:text-sky-200">← Back to account</button>
      ...
      {section === 'regional' ? (
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          ...
          <label className="block text-sm text-slate-200">
            ...
            <datalist id="profile-currencies">{currencies.map((currency) => <option key={currency} value={currency}>{getCurrencyLabel(currency)}</option>)}</datalist>
          </label>
        </div>
      ) : null}
      ...
      {section === 'preferences' ? (
        <div className="mt-6 space-y-4">
          ...
          {(['hotelPreferences', 'dietaryRequirements', 'accessibilityNeeds'] as const).map((field) => {
            const labels = { hotelPreferences: 'Hotel preferences', dietaryRequirements: 'Dietary requirements', accessibilityNeeds: 'Accessibility needs' };
            return <label key={field} className="block text-sm text-slate-200"><span className="mb-2 block">{labels[field]}</span><textarea rows={3} className={inputClass} value={profile[field]} onChange={(event) => update(field, event.target.value)} /></label>;
          })}
        </div>
      ) : null}
